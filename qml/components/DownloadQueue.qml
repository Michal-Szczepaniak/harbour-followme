...
Item {
   ...
   function downloadNext() {
       ...
       console.log("next locator has length: " + item['locator'].length, JSON.stringify(item['locator']));
       ...
       if (item['remoteFile'] != undefined) {
           ...
           // downloaders needs the page locator (not the actual image), iow: the parent
                                 if(item['locator'].length == 4){
               downloader.locator = item['locator']
           } else{
   ...
   onFetchDone: {
       ...
       if (item['sort'] != undefined && entries.length > 1) {
           console.log('sorting the items properly', item['locator'].length);
           console.log('before sort: ' + entries[1].id);
       if(item['locator'].length == 1) entries.sort(item['sort']);
