...
Ajax {
    ...
    onStarted: {
        if (searchName != '' && plugin.search != undefined && plugin.search != false) {
            ...
            if(JSON.stringify(locator).indexOf('manga-') > 0) locator = JSON.parse(JSON.stringify(locator).replace('manga-','manga/'))
                        console.log(JSON.stringify(locator))
            url = Utils.getURL(plugin, locator);
            url = url.replace(/%3A/gm, ':');
        }
    }

    onFinished: {
        if (status != 200) {
            console.log('xhr status: ' + status, url);
            done(false,[]);
            return ;
        }
        ...
        // check preFilter
        console.log('checking preFilter: ', level.preFilter);
        ...
        //        console.log(remoteFile);
                if (remoteFile.indexOf('http') == -1) remoteFile = 'http:' + remoteFile
