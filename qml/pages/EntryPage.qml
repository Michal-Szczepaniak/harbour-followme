...
anchors.fill: parent
    ...
    onChapterLoaded: {
        ...
        //	for (var i in chapter.items) {
            //    console.log(chapter.items[i].id, parentEntry.currentPage);
    ...
    onCalcCurrentCompletion: {
        if (parentEntry.items.length > 0) {
            if (entryView.count <= 1 || entryView.indexAt(0, entryView.contentY + entryView.height) == entryView.count - 1) {
                ...
                console.log(entryView.indexAt(0, entryView.contentY + entryView.height), entryView.height,entryView.indexAt(0, entryView.contentY), entryView.indexAt(0,  3000), entryView.contentY, entryView.originY );
                if (currentPartIndex != undefined && parentEntry.items[currentPartIndex] != undefined && parentEntry.items.length != 0) {
                    ...
                    console.log(entryView.indexAt(0, entryView.contentY + entryView.height), entryView.height,entryView.indexAt(0, entryView.contentY))
                    console.log(parentEntry.items[currentPartIndex].id);
