...
def locateFolder(base, locator):
    ...
    if not os.path.exists(folder):
        return None
    return os.path.join(folder, '/'.join([x['id'].replace('/','-').replace('#','').replace('?','-') for x in locator]))
...
def downloadData(base, locator, suffix, remotefile, redownload):
    ...
    try:
        ...
        pass
    # get the filename
    name = fileitem['id'].replace('/','-').replace('#','').replace('?','-').replace('%', '') + suffix
    ...
    #    req.add_header('Accept', '*/*')
        if "zjcdn.mangahere" in remotefile:
           req.add_header('Accept', '*/*')
           req.add_header('Host', 'zjcdn.mangahere.org')
           req.add_header('Referer', 'http://www.mangatown.com')
